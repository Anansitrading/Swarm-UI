...
export function TerminalPane({
    ...
    // Sync with prop changes - when parent provides a new ptyId, use it
    useEffect(() => {
        if (ptyId && ptyId !== currentPtyId) {
            setCurrentPtyId(ptyId);
        }
    }, [ptyId, currentPtyId]);

    // Auto-spawn a PTY if none provided and no spawnConfig defers it
    useEffect(() => {
        if (!currentPtyId && spawnConfig) {
    ...
    return (
        <div
            className={`flex items-center justify-center bg-swarm-bg ${className ?? ""}`}
        >
